...
export default {
    ...
    getFee: async ({ kode_pos_pengirim, kode_pos_penerima, berat, panjang = 0, lebar = 0, tinggi = 0, diameter = 0, harga }) => {
        const token = await getToken();
        const data = {
    ...
    addPostingDoc: async ({ order_toko_id, alamat_pengirim, alamat_penerima, detail_item, detail_pembayaran, pajak, layanan }) => {
        const token = await getToken();
        const data = {
            "userid": 1,
